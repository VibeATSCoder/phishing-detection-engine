...
def test_component_versions_and_compose_are_synchronized():
    ...
    assert project_version and project_version.group(1) == __version__ == "3.8.1"

    contract = json.loads((ROOT / "deploy" / "COMPATIBILITY.json").read_text(encoding="utf-8"))
    assert contract["components"]["detector"]["version"] == "3.8.1"
    ...
    assert "phishing-detection-engine:3.8.1-integrated" in compose  # build-from-source tag
...
def test_clean_docker_build_does_not_require_runtime_database():
    ...
    assert 'ARG APP_VERSION=3.8.1' in dockerfile
